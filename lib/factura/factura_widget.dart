...
class _FacturaWidgetState extends State<FacturaWidget> {
  ...
  void initState() {
    super.initState();
    _model = createModel(context, () => FacturaModel());

    // On page load action.
    SchedulerBinding.instance.addPostFrameCallback((_) async {
      await Future.delayed(const Duration(milliseconds: 2000));
      _model.apiResultp2w = await VmcCall.call(
        id: widget.sn?.toString(),
      );
      if ((_model.apiResultp2w?.succeeded ?? true)) {
        await SendsalesCall.call(
          a: widget.idp?.toString(),
          b: widget.sn?.toString(),
          e: 0,
          f: widget.vmcprice,
          c: 'VMFINSUSAAA001',
          g: FFAppState().niv,
  ...
  Widget build(BuildContext context) {
    ...
    return FutureBuilder<ApiCallResponse>(
      ...
      builder: (context, snapshot) {
        ...
        final facturaInfoproductoResponse = snapshot.data!;
